...
def callback(request):
    ...
    if not code:
        return JsonResponse({'error': 'Código não encontrado na URL.'}, status=400)
    ...
    try:
        ...
        novos_lancamentos_raw = sp.new_releases(country="BR", limit=5)
        musicas = []
        for item in novos_lancamentos_raw['albums']['items']:
            musicas.append({
                'nome': item['name'],
                'artista': [artista['name'] for artista in item['artists']]
            })
        return JsonResponse({'Novos lancamentos': musicas})
